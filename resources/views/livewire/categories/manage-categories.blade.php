    ...
    <div class="sm:flex sm:items-center sm:justify-between mb-8">
        <div>
            ...
            <p class="mt-2 text-sm text-slate-400">
                Administra las categorías de productos de Tienda Dolores.
            </p>
        </div>
            ...
            <table class="min-w-full divide-y divide-slate-800">
                <thead>
                    <tr class="bg-slate-900/70">
                        ...
                        <th class="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider text-slate-400">Nombre</th>
                        <th class="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider text-slate-400">Slug (URL)</th>
                        <th class="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider text-slate-400">Descripción</th>
                        <th class="px-6 py-4 text-center text-xs font-semibold uppercase tracking-wider text-slate-400">Estado</th>
                    ...
                    @forelse ($categories as $category)
                        <tr class="hover:bg-slate-800/20 transition-colors duration-150">
                            ...
                            <td class="px-6 py-4 text-sm text-slate-300">
                                <div class="line-clamp-1 max-w-xs">{{ $category->description ?? 'Sin descripción' }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-center text-sm">
                                ...
                                @else
                                    <span class="inline-flex items-center rounded-full bg-slate-800 px-2.5 py-0.5 text-xs font-medium text-slate-400 border border-slate-700">
                                        Inactiva
                                    </span>
                                @endif
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-center text-sm">
                                <div class="flex items-center justify-center gap-2">
                                    <button wire:click="edit({{ $category->id }})" class="p-2 rounded-lg bg-slate-800 hover:bg-indigo-600 text-slate-400 hover:text-white transition-all duration-150" title="Editar">
                                    ...
                                    <button wire:click="delete({{ $category->id }})" wire:confirm="¿Estás seguro de eliminar esta categoría?" class="p-2 rounded-lg bg-slate-800 hover:bg-rose-600 text-slate-400 hover:text-white transition-all duration-150" title="Eliminar">
                                        <svg class="h-4.5 w-4.5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                        ...
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center text-slate-500">
                                <div class="flex flex-col items-center justify-center">
                                    ...
                                    <span class="text-sm font-medium">No se encontraron categorías</span>
                                </div>
                            </td>
                        </tr>
                    @endforelse
                ...
                <div class="relative transform overflow-hidden rounded-2xl bg-slate-900 border border-slate-800 text-left shadow-2xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
                    ...
                    <form wire:submit.prevent="save">
                        <div class="px-6 py-5 space-y-4">
                            <!-- Name -->
                            <div>
                                <label for="name" class="block text-xs font-semibold uppercase tracking-wider text-slate-400 mb-1.5">Nombre de la Categoría</label>
                                <input type="text" wire:model.live="name" id="name" class="block w-full rounded-xl border-0 bg-slate-950/60 py-2.5 px-3.5 text-white ring-1 ring-inset ring-slate-800 focus:ring-2 focus:ring-indigo-500 sm:text-sm focus:outline-none transition-all duration-200" placeholder="Ej. Abarrotes">
                                @error('name') <span class="text-rose-400 text-xs mt-1 block">{{ $message }}</span> @enderror
                            </div>

                            <!-- Slug -->
                            <div>
                                <label for="slug" class="block text-xs font-semibold uppercase tracking-wider text-slate-400 mb-1.5">Slug (URL única)</label>
                                <input type="text" wire:model="slug" id="slug" class="block w-full rounded-xl border-0 bg-slate-950/60 py-2.5 px-3.5 text-white ring-1 ring-inset ring-slate-800 focus:ring-2 focus:ring-indigo-500 sm:text-sm focus:outline-none transition-all duration-200 font-mono" placeholder="ej-abarrotes">
                            ...
                            <div>
                                ...
                                <textarea wire:model="description" id="description" rows="3" class="block w-full rounded-xl border-0 bg-slate-950/60 py-2.5 px-3.5 text-white ring-1 ring-inset ring-slate-800 focus:ring-2 focus:ring-indigo-500 sm:text-sm focus:outline-none transition-all duration-200" placeholder="Breve descripción..."></textarea>
                                @error('description') <span class="text-rose-400 text-xs mt-1 block">{{ $message }}</span> @enderror
                            </div>

                            <!-- Active status -->
                            <div class="flex items-center gap-3">
                                <input type="checkbox" wire:model="is_active" id="is_active" class="h-4 w-4 rounded border-slate-800 text-indigo-600 focus:ring-indigo-500 bg-slate-950/60">
                                <label for="is_active" class="text-sm text-slate-300">Categoría Activa (disponible para productos)</label>
                            </div>
                        </div>
